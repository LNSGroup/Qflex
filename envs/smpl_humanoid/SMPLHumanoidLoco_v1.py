...
from humenv.rewards import get_xpos
...
class HumEnvModifiedV1(HumEnv):
    ...
    def step(self, action: np.ndarray):
        ...
        if self.data.time >= self.terminate_time:
            truncated = True
        info = self.get_info()
        return observation, reward, terminated, truncated, info

    def is_terminated(self) -> bool:
        pelvis_height = get_xpos(self.model, self.data, name="Pelvis")[-1]
        head_height = get_xpos(self.model, self.data, name="Head")[-1]
        # print(pelvis_height, head_height)
        # print(f"pelvis height: {pelvis_height}")
        if pelvis_height < 0.4 and 'jump' not in self.task_name:
            return True
        elif head_height < 0.5 or pelvis_height < 0.2:
            return True
        else:
            return False
    ...
    def get_obs(self) -> Dict[str, np.ndarray]:
        ...
        return np.concatenate([v.ravel() for v in obs_dict.values()], axis=0, dtype=np.float64)
