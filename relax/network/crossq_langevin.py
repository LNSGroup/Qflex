...
class CrossQLangevinParams(NamedTuple):
    q1: hk.Params
    q2: hk.Params
    target_q1: hk.Params
    target_q2: hk.Params
...
class CrossQLangevinNet:
    ...
    def get_action(self, key: jax.Array, policy_params: hk.Params, state: hk.State, obs: jax.Array,
                   langevin_step_size: float = 1e-3, langevin_num_step: int = 1) -> jax.Array:
        """for data collection"""
        policy_params, q1_params, q2_params = policy_params
        policy_state, q1_state, q2_state = state
        
        (mean, std), policy_state = self.policy(policy_params, policy_state, obs, is_training=False)
        z = jax.random.normal(key, (mean.shape[0]*(langevin_num_step + 1), mean.shape[1]))
        act = mean + std * z[0]
        act = jnp.tanh(act)
        ...
        def langevin_update(
            ...
            epsilon = langevin_step_size * std
            def q_maximum(action, q1_params, q1_state, q2_params, q2_state, obs):
                q1_value, q1_state = self.q(q1_params, q1_state, obs, action, is_training=False)
                q2_value, q2_state = self.q(q2_params, q2_state, obs, action, is_training=False)
                return jnp.mean(jnp.maximum(q1_value, q2_value))
            action_grad = jax.grad(q_maximum)(action, q1_params, q1_state, q2_params, q2_state, obs)
            action = action + epsilon * action_grad + jnp.sqrt(2 * epsilon) * noise
    ...
    def get_deterministic_action(self, policy_params: hk.Params, state: hk.State, obs: jax.Array) -> jax.Array:
        ...
        return self.get_action(key, policy_params, state, obs) # implement like this for now TODO: change to deterministic version
...
def create_crossq_langevin_net(
    key: jax.Array,
    obs_dim: int,
    act_dim: int,
...
) -> Tuple[CrossQLangevinNet, CrossQLangevinParams]:
    ...
    def init(key, obs, act):
        ...
        return CrossQLangevinParams(q1_params, q2_params, target_q1_params, target_q2_params, policy_params, 
    ...
    print('init params')
    # raise NotImplementedError
    net = CrossQLangevinNet(policy=policy.apply, 
