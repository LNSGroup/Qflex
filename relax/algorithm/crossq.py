...
class CrossQ(Algorithm):
    def __init__(self, agent: CrossQNet, params: CrossQParams, *, gamma: float = 0.99, lr: float = 1e-4,
                 alpha_lr: float = 3e-4, 
        ...
        def stateless_update(
            ...
            state = CrossQTrainState(
                params=CrossQParams(q1_params, q2_params, target_q1_params, target_q2_params, policy_params, q1_state, q2_state, policy_state, log_alpha,
                                    # q1_eval_params, q2_eval_params, policy_eval_params, q1_eval_state, q2_eval_state, policy_eval_state
                                    ),
    ...
    def get_action(self, key: jax.Array, obs: np.ndarray) -> np.ndarray:
        policy_state = self.state.params.policy_state
        action = self._get_action(key, self.get_policy_params(), policy_state, obs)
        return np.asarray(action)

    def get_deterministic_action(self, obs: np.ndarray) -> np.ndarray:
        policy_state = self.state.params.policy_state
        action = self._get_deterministic_action(self.get_policy_params(), policy_state, obs)
        return np.asarray(action)
    ...
    def get_policy_state(self):
        return self.state.params.policy_state
