...
class SAC(Algorithm):
    def __init__(self, agent: SACNet, params: SACParams, *, gamma: float = 0.99, lr: float = 1e-4,
        ...
        def stateless_update(
            ...
            # wide_obs = jnp.repeat(next_obs, repeats=10, axis=0)
            # wide_act = jnp.repeat(next_action, repeats=10, axis=0)
            # wide_q1_target = self.agent.q(target_q1_params, wide_obs, wide_act)
            # .reshape((-1, 10))
            # wide_q1_target = wide_q1_target.reshape((-1, 10))
            # wide_q1_target = wide_q1_target[jnp.arange(wide_q1_target.shape[0]), jnp.zeros(wide_q1_target.shape[0], dtype=int)]
            # assert jnp.mean(q1_target[0] - wide_q1_target[0]) == 0, f"bug in jax v0.4.20 {q1_target[0]} {wide_q1_target[0]}"
